...
<style>
	...
	function pageSizeEdit(pageSize){
		location.href="<%=request.getContextPath()%>/admin/board/boardSearch_list.do?pageSize="+pageSize+"&pageNum=<%=pageNum%>&option=<%=option%>&value=<%=value%>";
	}
	...
	<body>
		...
		<%if (bDTOArr.size()>0){
				...
				if (startPage>pageBlock){%>
					<input type="button" value="<" class="btn btn-default" onClick="location.href='<%=request.getContextPath()%>/admin/board/boardSearch_list.do?pageNum=<%=startPage-pageBlock%>&pageSize=<%=pageSize%>&option=<%=option%>&value=<%=value%>'">
	<%		}
				for(int i=startPage; i<=endPage; ++i){%>
					<%if(Integer.parseInt(pageNum)==i){ %>
					<input type="button" value="<%=i %>" class="btn btn-primary"  onClick="location.href='<%=request.getContextPath()%>/admin/board/boardSearch_list.do?pageNum=<%=i%>&pageSize=<%=pageSize%>&option=<%=option%>&value=<%=value%>'">		
					<%}else{ %>
					<input type="button" value="<%=i %>" class="btn btn-default"  onClick="location.href='<%=request.getContextPath()%>/admin/board/boardSearch_list.do?pageNum=<%=i%>&pageSize=<%=pageSize%>&option=<%=option%>&value=<%=value%>'">		
					<%} %>
	<%		}
