...
<script>
	...
	function reple_Edit(review_seq){ // 답변 수정
		...
		if(check == false){
			$(r_text).replaceWith($(
					"<form id='r_obj"+review_seq+"' action='<%=request.getContextPath()%>/seller/ft/ft_review_reple_edit.do' method='post'>"+
...
<body>
	...
	<div align="center">
		<input type="button"  value="목록" class="btn btn-default" style="width:80px;" onClick="location.href='<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_list&ft_seq=<%=revDTO.getFt_seq()%>'">
		<input type="button"  value="수정" class="btn btn-default" style="width:80px;" onClick="location.href='<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_edit&ft_seq=<%=revDTO.getFt_seq()%>&review_seq=<%=revDTO.getReview_seq()%>'">
		<input type="button"  value="삭제" class="btn btn-default" style="width:80px;" onClick="location.href='<%=request.getContextPath()%>/seller/ft/ft_review_delete.do?cmd=review_list&ft_seq=<%=revDTO.getFt_seq()%>&review_seq=<%=revDTO.getReview_seq()%>'">
	</div>
