	...
	<head>
		...
		<script>
			function pageSizeEdit(pageSize){
				location.href="<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_issue_list&pageSize="+pageSize+"&pageNum=<%=pageNum%>";
			}
		...
		<div  id="edition"  align="center"  style="clear:both;">
        	<%if (cpisDTOarr.size()>0){
				...
				if (startPage>pageBlock){%>
					<input type="button" value="<" class="btn btn-default" onClick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_issue_list&pageNum=<%=startPage-pageBlock%>&pageSize=<%=pageSize%>'">
	<%		}
				for(int i=startPage; i<=endPage; ++i){%>
				<%if(Integer.parseInt(pageNum)==i){ %>
					<input type="button" value="<%=i %>" class="btn btn-primary"  onClick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_issue_list&pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%}else{ %>
					<input type="button" value="<%=i %>" class="btn btn-default"  onClick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_issue_list&pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%} %>
	<%		}
