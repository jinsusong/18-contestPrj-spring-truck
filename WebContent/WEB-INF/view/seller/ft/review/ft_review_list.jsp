...
<html>
	...
	function pageSizeEdit(pageSize){
		location.href="<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_list&ft_seq=<%=ftDTO2.getFt_seq()%>&pageSize="+pageSize+"&pageNum=<%=pageNum%>";
	}
	...
	function checkReview(mode){
		...
		if(mode==1 && check_Cnt != 0){
			location.href="<%=request.getContextPath()%>/seller/ft/ft_review_delete.do?cmd=review_list&ft_seq=<%=ftDTO2.getFt_seq()%>&ArrReview_Seq="+check_Arr;
		}else if(mode==2 && check_Cnt != 0){
			location.href="<%=request.getContextPath()%>/seller/ft/ft_review_exp_yn.do?cmd=review_list&ft_seq=<%=ftDTO2.getFt_seq()%>&ArrReview_Seq="+check_Arr;
		}
	}
	</script>
	...
	<body>
		...
		<div style="margin-top:10px;">
			...
			<div class="form-group" style="position:relative; float:right;">
	            	<input type="button"  value="리뷰쓰기"  class="btn btn-primary" onclick="location.href='<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_create&ft_seq=<%=ftDTO2.getFt_seq()%>'" > 
	        </div> 
	     </div>
		<div  id="edition"  align="center"  style="clear:both;">
        	<%if (revDTOarr.size()>0){
				...
				if (startPage>pageBlock){%>
					<input type="button" value="<" class="btn btn-default" onClick="location.href='<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_list&ft_seq=<%=ftDTO2.getFt_seq()%>&pageNum=<%=startPage-pageBlock%>&pageSize=<%=pageSize%>''">
	<%		}
				for(int i=startPage; i<=endPage; ++i){%>
				<%if(Integer.parseInt(pageNum)==i){ %>
					<input type="button" value="<%=i %>" class="btn btn-primary"  onClick="location.href='<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_list&ft_seq=<%=ftDTO2.getFt_seq()%>&pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%}else{ %>
					<input type="button" value="<%=i %>" class="btn btn-default"  onClick="location.href='<%=request.getContextPath()%>/seller/ft/ft_info.do?cmd=review_list&ft_seq=<%=ftDTO2.getFt_seq()%>&pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%} %>
	<%		}
