...
<html>
	<head>
		...
		<link rel="stylesheet" href="<%=request.getContextPath() %>/resources/css/editablegrid-1.0.10.css"  type="text/css" />
		...
		<script>
			function pageSizeEdit(pageSize){
				location.href="<%=request.getContextPath()%>/ft/ft_list.do?pageSize="+pageSize+"&pageNum=<%=pageNum%>";
			}
			...
			function checkFT(mode){
				...
				if(mode==1 && check_Cnt != 0){
					location.href="<%=request.getContextPath()%>/ft/ft_active.do?ArrFt_Seq="+check_Arr;
				}else if(mode==2 && check_Cnt != 0){
					location.href="<%=request.getContextPath()%>/ft/ft_drop.do?ArrFt_Seq="+check_Arr;
				}
			}
		</script>
	</head>
        ...
        <table id="htmlgrid" class="testgrid" style="margin-top:10px;">
			...
			<%
				...
				for(int i=startCnt; i<endCnt; i++){%>
					...
					<td  style="cursor:pointer;" onclick="location.href='<%=request.getContextPath()%>/ft/ft_info.do?ft_seq=<%=ftDTOArr.get(i).getFt_seq()%>'">
		...
		<div  id="edition"  align="center"  style="clear:both;">
        	<%if (ftDTOArr.size()>0){
				...
				if (startPage>pageBlock){%>
					<input type="button" value="<" class="btn btn-default" onClick="location.href='<%=request.getContextPath()%>/ft/ft_list.do?pageNum=<%=startPage-pageBlock%>&pageSize=<%=pageSize%>''">
	<%		}
				for(int i=startPage; i<=endPage; ++i){%>
				<%if(Integer.parseInt(pageNum)==i){ %>
					<input type="button" value="<%=i %>" class="btn btn-primary"  onClick="location.href='<%=request.getContextPath()%>/ft/ft_list.do?pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%}else{ %>
					<input type="button" value="<%=i %>" class="btn btn-default"  onClick="location.href='<%=request.getContextPath()%>/ft/ft_list.do?pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%} %>
	<%		}
