...
<html>
	<head>
		...
		<script>
			function pageSizeEdit(pageSize){
				location.href="<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_search_list&pageSize="+pageSize+"&pageNum=<%=pageNum%>&option=<%=option%>&value=<%=value%>";
			}
			...
			function checkCoupon(mode){
				...
				if(mode==1 && check_Cnt != 0){
					location.href="<%=request.getContextPath()%>/admin/coupon/coupon_delete.do?cmd=coupon_search_list&ArrCoupon_Code="+check_Arr+"&option=<%=option%>&value=<%=value%>";
				}
			}
		</script>
	</head>
    ...
    <div style="width:100%;">
		...
		<div style="float:right; margin-top:20px;">
			<input type="button" class="btn btn-default" value="뒤로가기" onclick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do'">
		</div>
	</div>
		...
		<div  id="edition"  align="center"  style="clear:both;">
        	<%if (cpDTOarr.size()>0){
				...
				if (startPage>pageBlock){%>
					<input type="button" value="<" class="btn btn-default" onClick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_search_list&pageNum=<%=startPage-pageBlock%>&pageSize=<%=pageSize%>&option=<%=option%>&value=<%=value%>'">
	<%		}
				for(int i=startPage; i<=endPage; ++i){%>
				<%if(Integer.parseInt(pageNum)==i){ %>
					<input type="button" value="<%=i %>" class="btn btn-primary"  onClick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_search_list&pageNum=<%=i%>&pageSize=<%=pageSize%>&option=<%=option%>&value=<%=value%>'">		
				<%}else{ %>
					<input type="button" value="<%=i %>" class="btn btn-default"  onClick="location.href='<%=request.getContextPath()%>/admin/coupon/coupon_main.do?cmd=coupon_search_list&pageNum=<%=i%>&pageSize=<%=pageSize%>&option=<%=option%>&value=<%=value%>'">		
				<%} %>
	<%		}
